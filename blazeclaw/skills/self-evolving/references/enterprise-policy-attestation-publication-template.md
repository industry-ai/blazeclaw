...
- Automation command (shell):
  `scripts/outage-outcome-promoter.sh --simulation-id <id> --tenant-id <tenant-id> --rollout-phase <r1|r2|r3|r4> --policy-profile <profile-id> --strict-schema-version <version-optional> --require-signed-manifest --manifest-file <manifest-path-optional> --signature-verification-mode <none|kms|sigstore> --require-trust-policy --trust-policy-file <trust-policy-path-optional> --require-revocation-check --revocation-file <revocation-path-optional> --require-trust-policy-attestation --trust-policy-attestation-file <attestation-path-optional> --require-revocation-slo --revocation-slo-file <revocation-slo-path-optional> --attestation-baseline-window <n> --attestation-anomaly-threshold-percent <0-100> --require-attestation-baseline-gate --tenant-criticality-file <criticality-path-optional> --adaptive-threshold-policy-file <tier-threshold-path-optional> --require-adaptive-threshold-policy --time-decay-policy-file <time-decay-policy-path-optional> --time-decay-half-life <samples> --require-time-decay-policy --recurrence-tuning-policy-file <recurrence-policy-path-optional> --require-recurrence-tuning-policy --seasonal-decomposition-policy-file <seasonal-policy-path-optional> --reporting-cycle-length <samples> --require-seasonal-decomposition-policy --seasonal-overlay-policy-file <overlay-policy-path-optional> --require-seasonal-overlay-policy --causal-clustering-policy-file <causal-policy-path-optional> --require-causal-clustering-policy --causal-graphing-policy-file <causal-graph-policy-path-optional> --require-causal-graphing-policy --graph-cohort-window <samples> --graph-temporal-decay-rate <non-negative-number> --explainer-drift-threshold <non-negative-int> --confidence-bound-zscore <positive-number> --bayes-prior-alpha <positive-number> --bayes-prior-beta <positive-number> --graph-explainability-file <graph-explainability-path-optional> --drift-root-cause-file <drift-root-cause-path-optional> --overlay-candidate-file <overlay-candidate-path-optional> --causal-graph-file <causal-graph-path-optional> --attestation-dashboard-file <dashboard-path-optional> --attestation-history-file <history-path-optional> --cross-tenant-heatmap-file <heatmap-path-optional> --auto-remediation-routing-file <routing-path-optional> --dependency <registry|authority> --result <pass|fail> --evidence-path <path>`
...
- Drift root-cause narrative path:
...
- Sigstore verifier dependency: `cosign`
...
- Signed manifest rule: when manifest gate is enabled, manifest fields and
  weight digest verification must pass or promotion is blocked.
- Cryptographic verification rule: when `kms` or `sigstore` mode is enabled,
  signature scheme match and verifier command success are required or
  promotion is blocked.
- Trust-policy distribution rule: when enabled, distribution freshness and
...
- Adaptive calibration rule: tenant criticality tiers should override static
  anomaly threshold when tier policy mapping is available.
- Time-decay rule: recency-weighted anomaly percent should be used for
  drift-sensitive baseline evaluation when weighting is enabled.
- Recurrence auto-tuning rule: decay half-life should be tuned from
  tenant incident recurrence ratio using policy-defined bounds.
- Seasonal decomposition rule: cycle-phase multipliers should tune
  recurrence-adjusted half-life according to reporting cycle position.
- Overlay rule: holiday/event overlays should compose on top of seasonal
  multipliers for phase-aware cycle exceptions.
- Causal clustering rule: causal clusters with sufficient samples and
  fail-rate impact should auto-suggest overlay candidates for review.
- Confidence graphing rule: multi-factor causal confidence must exceed
  policy threshold before suggested overlay candidates are promoted.
- Temporal persistence rule: cohort-window temporal decay should weight
  edge persistence before confidence-qualified promotion decisions.
- Explainability rule: factor-level contribution traces should be
  retained for recommendation auditability across cohorts.
- Explainer diffing rule: consecutive cohort contribution deltas should
  be compared and drift-flagged when exceeding approved threshold.
- Root-cause synthesis rule: drift-flagged cohort deltas should include
  dominant-factor narratives for audit-ready investigation trails.
- Evidence confidence rule: root-cause narratives should include
  probabilistic confidence bounds with explicit z-score assumptions.
- Bayesian evidence rule: posterior updates should publish prior
  assumptions (alpha/beta) alongside interval estimates.
