...
function parseAccountFromArgv(argv) {
    ...
    if (idx !== -1 && idx + 1 < args.length) {
        ...
        if (!/^[a-zA-Z0-9_-]{1,32}$/.test(name || '')) {
            console.error('Error: Invalid account name format. Use letters, numbers, underscore, or hyphen (max 32 chars).');
            process.exit(1);
        }
        args.splice(idx, 2);
...
function validateRequiredCredentials(cfg) {
    const errors = [];

    if (!cfg.imap.user) errors.push('IMAP_USER missing');
    if (!cfg.imap.pass) errors.push('IMAP_PASS missing');
    if (!cfg.smtp.host) errors.push('SMTP_HOST missing');
    if (!cfg.smtp.user) errors.push('SMTP_USER missing');
    if (!cfg.smtp.pass) errors.push('SMTP_PASS missing');

    if (errors.length > 0) {
        const accountHint = prefix ? ` for account "${prefix.toLowerCase()}"` : '';
        console.error(`Error: Incomplete email credentials${accountHint}. ${errors.join(', ')}`);
        process.exit(1);
    }
}
...
const activeCommand = remainingArgs[0] || '';
...
if (activeCommand !== 'list-accounts') {
