...
- `blazeclaw/docs/SKILL_PORTING.md` — Phase F skill PORTING_PLAN pattern; **`Verify-SkillPortingPlans.ps1`** + Catch2 **`[skills][phasef][contract]`**.
...
- `BuildGatewayParityLifecycleTraceJson()` (Phases S0–S4, OpenClaw `server.impl.ts` parity)

Builds **`OperatorDiagnosticsInputs`** (projector contexts + scalar fields from live `ServiceManager` state) and delegates snapshot assembly and report text to **`OperatorDiagnosticsAssembler`** → `CDiagnosticsReportBuilder`. The operator JSON report embeds **`GatewayParityLifecycleContract`** (schema **v5**: S1–S2 as before; **S4** adds `extensionSurfaceReloadCount`, `lastAppliedExtensionSurfaceEpoch`, `lastExtensionSurfaceMethodDeltaJson`; **S5** adds `gatewayShutdownInvocationCount` and nested **`lastShutdownPrelude`** — `recordedAtEpochMs`, `snapshotCleanupPathUtf8`, `phaseOrderUtf8`, `pluginGlobalStopInvoked` — for last normal stop or startup-failure cleanup) under **`runtime.gatewayLifecycle.parityContract`**. The same contract is returned (wrapped with `ok` / `contract`) from the gateway RPC **`gateway.parity.lifecycle`** after **`GatewayHostBindingCoordinator::WireAllGatewayServiceCallbacks`** sets **`GatewayHost::SetParityLifecycleExportCallback`**. At **`FinalizeStartup`**, **`RecordGatewayStartupConfigSnapshot`**, **`RefreshGatewayAuthBootstrapDiagnostics`**, and **`startupResult.resolvedRuntime` → `resolvedRuntimeConfig`** feed projector inputs; **`GatewayRuntimeBootstrapCoordinator`** runs S1 migrations, then **`ResolveGatewayRuntimeConfig`** (trace **`GatewayRuntimeBootstrap.ResolveRuntimeConfig.ready`**) for S2. **S3** centralizes a non-owning **`GatewayRuntimeContext`** projection in **`GatewayHost`**, bound in **`RegisterDefaultHandlers`** / **`RegisterRuntimeHandlers`** and cleared in **`Stop`**, so transport and handler modules share explicit dispatcher/transport/session/tool/fanout/node pointers. **S4:** managed config apply bumps **`embedded.extensionSurfaceApplyEpoch`** to trigger **`GatewayHost::PerformDeferredExtensionCatalogReloadWithMethodSurfaceTelemetry`** (method-surface snapshot + extension/plugin registry resync). Runtime finalize also runs **`GatewayMethodSurfaceAudit`**: generated catalog and channel RPC names must be registered, and the **union of optional `gatewayRpcMethods`** from extension manifests must be registered (`plugin_rpc_surface_not_registered` if not). **S5:** **`Stop`** / startup-failure cleanup record shutdown phase ordering into **`lastShutdownPrelude`**; **`plugin_global_stop`** owned cleanup invokes **`GatewayHost::NotifyPluginGlobalStopPrelude`** before **`gateway_host_stop`**.
...
23. **Gateway host binding coordinator (`GatewayHostBindingCoordinator`)**
   ...
   - **`ServiceManager`** declares **`friend class GatewayHostBindingCoordinator`** and **`friend class ChatRuntimeOrchestrationCoordinator`**. **`GatewayHostBindingCoordinator`** implements **`BindGatewayPolicyCallbacks`** / **`BindEmbeddingsCallbacks`** (embedded/email policy + embeddings **`Set*Callback`** wiring); only **`BindToolRuntimeCallbacks`** remains on **`ServiceManager`** (TU-local runtime tool helpers). **`WireAllGatewayServiceCallbacks`** sequences all of them in order.
...
9. **Introduce internal wiring helpers with strict naming**
   - Example: coordinator **`BindGatewayPolicyCallbacks`** / **`BindEmbeddingsCallbacks`**, **`ServiceManager::BindToolRuntimeCallbacks`**, plus **`RegisterSkillsRelatedCallbacks`** / **`RegisterChatRuntimeCallbacks`**.
   - Easier code navigation and onboarding.
   - ✅ Implemented strict-named internal wiring helper decomposition.
   - **`WireGatewayCallbacks()`** delegates to **`GatewayHostBindingCoordinator::WireAllGatewayServiceCallbacks`**, which invokes **`RegisterSkillsRelatedCallbacks`**, **`BindGatewayPolicyCallbacks`**, **`BindToolRuntimeCallbacks`** (on **`ServiceManager`**), **`RegisterChatRuntimeCallbacks`**, **`BindEmbeddingsCallbacks`** in deterministic order (see **`SERVICE_LAYER_BOUNDARIES.md`** / **`GATEWAY_CORE_WIRING.md`** Phase C).
