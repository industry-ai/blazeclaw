...
int QrSegment::getTotalBits(const vector<QrSegment>& segs, int version) {
	...
	for (const QrSegment& seg : segs) {
		...
		if (4 + ccbits > INT_MAX - result)
			return -1;
		result += 4 + ccbits;
		if (seg.data.size() > static_cast<size_t>(INT_MAX - result))
			return -1;
...
QrCode QrCode::encodeSegments(const vector<QrSegment>& segs, Ecc ecl,
	...
	assert(bb.size() == static_cast<size_t>(dataUsedBits));
...
vector<uint8_t> QrCode::addEccAndInterleave(const vector<uint8_t>& data) const {
	if (data.size() != static_cast<size_t>(getNumDataCodewords(version, errorCorrectionLevel)))
	...
	for (int i = 0, k = 0; i < numBlocks; i++) {
		...
		vector<uint8_t> dat(data.cbegin() + k, data.cbegin() + (k + datLen));
		k += datLen;
	...
	assert(result.size() == static_cast<size_t>(rawCodewords));
	return result;
}

void QrCode::drawCodewords(const vector<uint8_t>& data) {
	if (data.size() != static_cast<size_t>(getNumRawDataModules(version) / 8))
		throw std::invalid_argument("Invalid argument");
