...
namespace {
		...
		const std::string& propertiesJson)
	{
		...
		html
			...
			<< "<div id=\"generated-warning\" style=\"display:none;margin-top:10px;padding:10px;border:1px solid #e5b400;background:#fff7cc;color:#6b5600;border-radius:6px;\"></div>"
			<< "<div id=\"generated-status\" style=\"margin-top:10px;color:#444;\"></div>"
		...
		html
			...
			<< "const envContainer = document.getElementById('env-fields');\n"
			<< "const cfgContainer = document.getElementById('config-fields');\n"
			<< "const warningElement = document.getElementById('generated-warning');\n"
			<< "const statusElement = document.getElementById('generated-status');\n"
			<< "document.getElementById('skill-key').textContent = skillKey;\n"
			<< "hintsElement.textContent = JSON.stringify(configHints, null, 2);\n"
			...
			<< "    const id = 'generated-rpc-' + Date.now() + '-' + Math.random().toString(36).slice(2);\n"
			<< "    pendingRpc.set(id, resolve);\n"
			<< "    send({ channel: 'blazeclaw.gateway.rpc', id, method, params: params || {} });\n"
			<< "  });\n"
			<< "}\n"
			<< "function fieldElementId(key) {\n"
			<< "  return 'f_' + key.replace(/[^a-zA-Z0-9_]/g, '_');\n"
			<< "}\n"
			<< "function addField(container, key, hint) {\n"
			<< "  const label = document.createElement('label');\n"
			<< "  label.textContent = (hint && hint.label) ? hint.label : key;\n"
			<< "  const input = document.createElement('input');\n"
			<< "  input.id = fieldElementId(key);\n"
			<< "  if (hint && hint.placeholder) {\n"
			<< "    input.placeholder = hint.placeholder;\n"
			<< "  }\n"
			<< "  if (hint && hint.sensitive) {\n"
			<< "    input.type = 'password';\n"
			<< "  }\n"
			...
			<< "  if (message.channel !== 'blazeclaw.gateway.rpc.result' || !message.id) {\n"
			<< "    return false;\n"
			<< "  }\n"
			...
			<< "  return true;\n"
			<< "}\n"
			<< "function applyLoadedPayload(message) {\n"
			<< "  if (message.channel !== 'blazeclaw.skill.config.loaded' || message.skillKey !== skillKey) {\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  const payload = message.payload || {};\n"
			<< "  Object.keys(payload).forEach((key) => {\n"
			<< "    state[key] = payload[key];\n"
			<< "    const element = document.getElementById(fieldElementId(key));\n"
			<< "    if (element) {\n"
			...
			<< "  if (handleRpcResult(message)) {\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  applyLoadedPayload(message);\n"
			<< "  if (message.channel === 'blazeclaw.skill.config.loaded' && message.skillKey === skillKey) {\n"
			<< "    setStatus('Loaded existing configuration.');\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  if (message.channel === 'blazeclaw.skill.config.saved' && message.skillKey === skillKey) {\n"
			<< "    setStatus('Saved to ' + (message.configPath || '(unknown path)'));\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  if (message.channel === 'blazeclaw.skill.config.validation' && message.skillKey === skillKey) {\n"
			<< "    setStatus(message.ok ? 'Validation passed.' : 'Validation failed.');\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  if (message.channel === 'blazeclaw.skill.config.error' && message.skillKey === skillKey) {\n"
			<< "    setStatus('Error: ' + (message.message || 'operation failed'));\n"
			<< "  }\n"
			<< "});\n"
			<< "async function bootstrapSchema() {\n"
			<< "  let uiHints = {};\n"
			<< "  const schemaState = await rpc('gateway.config.schema.get', {});\n"
			<< "  if (schemaState && schemaState.ok && schemaState.payload && schemaState.payload.uiHints) {\n"
			<< "    uiHints = schemaState.payload.uiHints;\n"
			<< "    hintsElement.textContent = JSON.stringify(uiHints, null, 2);\n"
			<< "  }\n"
			<< "  requiresEnv.forEach((key) => {\n"
			<< "    addField(envContainer, key, uiHints[key] || null);\n"
			<< "  });\n"
			<< "  for (const path of requiresConfig) {\n"
			<< "    let hint = uiHints[path] || null;\n"
			<< "    const lookup = await rpc('gateway.config.schema.lookup', { path });\n"
			<< "    if (lookup && lookup.ok && lookup.payload && lookup.payload.hint) {\n"
			<< "      hint = lookup.payload.hint;\n"
			<< "    }\n"
			<< "    addField(cfgContainer, path, hint);\n"
			<< "  }\n"
			<< "  if (requiresConfig.length === 0) {\n"
			<< "    configHints.forEach((path) => {\n"
			<< "      if (typeof path === 'string') {\n"
			<< "        addField(cfgContainer, path, uiHints[path] || null);\n"
			<< "      }\n"
			<< "    });\n"
			<< "  }\n"
			<< "  const totalFields = envContainer.querySelectorAll('input').length + cfgContainer.querySelectorAll('input').length;\n"
			<< "  if (totalFields === 0) {\n"
			<< "    showWarning('No configuration fields were exposed for this skill. Check skill metadata (primaryEnv/requiresEnv/requiresConfig/configPathHints) and reopen the page.');\n"
			<< "    setStatus('No editable fields found.');\n"
			<< "  } else {\n"
			<< "    showWarning('');\n"
			<< "    setStatus('Ready.');\n"
			<< "  }\n"
			...
			<< "    setStatus('Nothing to save. Fill at least one field before saving.');\n"
			<< "    return;\n"
			<< "  }\n"
			<< "  setStatus('Saving...');\n"
			<< "  send({ channel: 'blazeclaw.skill.config.save', skillKey, id: 'generated-save', payload: state });\n"
			<< "};\n"
			<< "document.getElementById('btn-validate').onclick = () => {\n"
			<< "  setStatus('Validating...');\n"
			<< "  send({ channel: 'blazeclaw.skill.config.validate', skillKey, id: 'generated-validate', payload: state });\n"
			<< "};\n"
			<< "document.getElementById('btn-cancel').onclick = () => {\n"
			<< "  setStatus('Cancelled.');\n"
			<< "  send({ channel: 'blazeclaw.skill.config.cancel', skillKey, id: 'generated-cancel' });\n"
			<< "};\n"
			<< "bootstrapSchema();\n"
			<< "</script></body></html>";
