...
namespace {
		...
		const std::string& propertiesJson)
	{
		...
		html
			...
			<< "l.textContent=(hint&&hint.label)?hint.label:key;"
			<< "const i=document.createElement('input');i.id='f_'+key.replace(/[^a-zA-Z0-9_]/g,'_');"
			<< "if(hint&&hint.placeholder){i.placeholder=hint.placeholder;}"
			<< "if(hint&&hint.sensitive){i.type='password';}"
			...
			<< "if(m.channel==='blazeclaw.gateway.rpc.result'&&m.id){"
			<< "const complete=pendingRpc.get(m.id);"
			<< "if(complete){pendingRpc.delete(m.id);complete(m);}"
			<< "return;"
			<< "}"
			...
			<< "document.getElementById('hints').textContent=JSON.stringify(uiHints,null,2);"
			<< "}"
			<< "requiresEnv.forEach(k=>addField(envContainer,k,uiHints[k]||null));"
			<< "for(const path of requiresConfig){"
			<< "let hint=uiHints[path]||null;"
			<< "const lookup=await rpc('gateway.config.schema.lookup',{path});"
			<< "if(lookup&&lookup.ok&&lookup.payload&&lookup.payload.hint){hint=lookup.payload.hint;}"
			<< "addField(cfgContainer,path,hint);"
			<< "}"
			<< "if(requiresConfig.length===0){"
			<< "configHints.forEach(path=>{if(typeof path==='string'){addField(cfgContainer,path,uiHints[path]||null);}});"
			<< "}"
