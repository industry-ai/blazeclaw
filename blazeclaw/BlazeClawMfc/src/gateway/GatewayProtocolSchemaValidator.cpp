...
namespace blazeclaw::gateway::protocol {
	...
	bool GatewayProtocolSchemaValidator::ValidateRequest(const RequestFrame& request, SchemaValidationIssue& issue) {
		...
		if (request.method == "gateway.protocol.version" ||
			...
			request.method == "gateway.config.manifest" ||
			request.method == "gateway.config.index" ||
			request.method == "gateway.config.state" ||
			...
			request.method == "gateway.transport.policy.reconcile" ||
			request.method == "gateway.transport.policy.sync" ||
			request.method == "gateway.transport.policy.refresh" ||
			...
			request.method == "gateway.events.pointer" ||
			request.method == "gateway.events.token" ||
			request.method == "gateway.events.stream" ||
			...
			request.method == "gateway.models.registry" ||
			request.method == "gateway.models.index" ||
			request.method == "gateway.models.state" ||
			...
			request.method == "gateway.tools.dispatch" ||
			request.method == "gateway.tools.router" ||
			request.method == "gateway.tools.selector" ||
	...
	bool GatewayProtocolSchemaValidator::ValidateResponseForMethod(
		...
		if (method == "gateway.events.stream") {
			if (!IsFieldValueType(payload, "stream", '"') || !IsFieldValueType(payload, "event", '"')) {
				SetIssue(issue, "schema_invalid_response", "`gateway.events.stream` requires `stream` string and `event` string.");
				return false;
			}
		...
		if (method == "gateway.events.token") {
			if (!IsFieldValueType(payload, "token", '"') || !IsFieldValueType(payload, "event", '"')) {
				SetIssue(issue, "schema_invalid_response", "`gateway.events.token` requires `token` string and `event` string.");
				return false;
			}
		...
		if (method == "gateway.models.state") {
			if (!IsFieldValueType(payload, "models", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.models.state` requires `models` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.events.pointer") {
			if (!IsFieldValueType(payload, "pointer", '"') || !IsFieldValueType(payload, "event", '"')) {
				SetIssue(issue, "schema_invalid_response", "`gateway.events.pointer` requires `pointer` string and `event` string.");
				return false;
			}
		...
		if (method == "gateway.tools.selector") {
			if (!IsFieldNumber(payload, "selected") || !IsFieldNumber(payload, "fallback") || !IsFieldNumber(payload, "tools")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.tools.selector` requires numeric fields `selected`, `fallback`, and `tools`.");
				return false;
			}
		...
		if (method == "gateway.models.index") {
			if (!IsFieldValueType(payload, "models", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.models.index` requires `models` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.transport.policy.refresh") {
			if (!IsFieldBoolean(payload, "refreshed") || !IsFieldNumber(payload, "version") || !IsFieldBoolean(payload, "applied")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.transport.policy.refresh` requires `refreshed` boolean, `version` number, and `applied` boolean.");
				return false;
			}
		...
		if (method == "gateway.config.state") {
			if (!IsFieldValueType(payload, "sections", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.config.state` requires `sections` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.tools.router") {
			if (!IsFieldNumber(payload, "routed") || !IsFieldNumber(payload, "fallback") || !IsFieldNumber(payload, "tools")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.tools.router` requires numeric fields `routed`, `fallback`, and `tools`.");
				return false;
			}
		...
		if (method == "gateway.models.registry") {
			if (!IsFieldValueType(payload, "models", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.models.registry` requires `models` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.transport.policy.sync") {
			if (!IsFieldBoolean(payload, "synced") || !IsFieldNumber(payload, "version") || !IsFieldBoolean(payload, "applied")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.transport.policy.sync` requires `synced` boolean, `version` number, and `applied` boolean.");
				return false;
			}
		...
		if (method == "gateway.events.marker") {
			if (!IsFieldValueType(payload, "marker", '"') || !IsFieldValueType(payload, "event", '"')) {
				SetIssue(issue, "schema_invalid_response", "`gateway.events.marker` requires `marker` string and `event` string.");
				return false;
			}
		...
		if (method == "gateway.config.index") {
			if (!IsFieldValueType(payload, "keys", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.config.index` requires `keys` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.tools.dispatch") {
			if (!IsFieldNumber(payload, "queued") || !IsFieldNumber(payload, "dispatched") || !IsFieldNumber(payload, "tools")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.tools.dispatch` requires numeric fields `queued`, `dispatched`, and `tools`.");
				return false;
			}
		...
		if (method == "gateway.transport.policy.reconcile") {
			if (!IsFieldBoolean(payload, "reconciled") || !IsFieldNumber(payload, "version") || !IsFieldBoolean(payload, "applied")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.transport.policy.reconcile` requires `reconciled` boolean, `version` number, and `applied` boolean.");
				return false;
			}
		...
		if (method == "gateway.config.manifest") {
			if (!IsFieldValueType(payload, "manifest", '"') || !IsFieldValueType(payload, "sections", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.config.manifest` requires `manifest` string, `sections` array, and `count` number.");
				return false;
			}
		...
		if (method == "gateway.models.inventory") {
			if (!IsFieldValueType(payload, "models", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.models.inventory` requires `models` array and `count` number.");
				return false;
			}
		...
		if (method == "gateway.tools.window") {
			if (!IsFieldNumber(payload, "windowSec") || !IsFieldNumber(payload, "calls") || !IsFieldNumber(payload, "tools")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.tools.window` requires numeric fields `windowSec`, `calls`, and `tools`.");
				return false;
			}
		...
		if (method == "gateway.transport.policy.apply") {
		...
		if (method == "gateway.config.package") {
			if (!IsFieldValueType(payload, "package", '"') || !IsFieldValueType(payload, "sections", '[') || !IsFieldNumber(payload, "count")) {
				SetIssue(issue, "schema_invalid_response", "`gateway.config.package` requires `package` string, `sections` array, and `count` number.");
				return false;
			}
