...
namespace blazeclaw::gateway::protocol {
	namespace {
		...
		std::string SerializeStateVersion(const StateVersion& stateVersion) {
			std::string output = "{";
			bool first = true;

			if (stateVersion.presence.has_value()) {
				AppendJsonUInt64(output, "presence", stateVersion.presence.value(), first);
			}

			if (stateVersion.health.has_value()) {
				AppendJsonUInt64(output, "health", stateVersion.health.value(), first);
			}
	...
	std::string SerializeEventFrame(const EventFrame& frame) {
		...
		if (frame.stateVersion.has_value()) {
			AppendJsonRaw(output, "stateVersion", SerializeStateVersion(frame.stateVersion.value()), first);
		}
