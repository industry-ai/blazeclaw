...
namespace blazeclaw::gateway {
	...
	std::string GatewayHost::HandleInboundText(const std::string& inboundJson) const {
		return GatewayHostProtocolIngressPipeline::ExecuteInboundTextPipeline(
			inboundJson,
			GatewayHostProtocolIngressPipeline::PipelineContext{
				.requestPolicyGuard = &m_requestPolicyGuard,
				.dispatchInitialized = m_dispatchInitialized,
				.hostRunning = m_running,
				.routeRequest = [this](const protocol::RequestFrame& request) {
					return RouteRequest(request);
				},
				.emitTelemetry = [](const std::string& eventName, const std::string& payloadJson) {
					EmitTelemetryEvent(eventName, payloadJson);
				},
			});
	}
	...
	void GatewayHost::WireCronProductionIntegration() {
		GatewayHostCronProductionAdapter::AdapterContext context;
		...
		context.transportRecipientRegistry = &m_transportRecipientRegistry;
		context.taskDeltaRepository = &m_taskDeltaRepository;
		context.eventFanoutService = &m_eventFanoutService;
		context.chatPushEventSeq = &m_chatPushEventSeq;
		context.cronPushEventSeq = &m_cronPushEventSeq;
		context.transportRunning = m_transport.IsRunning();

		m_cronAdapter.WireIntegration(context);
	}
	...
	bool GatewayHost::IsCronChatSessionBusy(const std::string& sessionKey) const {
		...
		context.chatRunsByIdPtr = static_cast<const void*>(&m_chatRunsById);
		context.transportRecipientRegistry = &m_transportRecipientRegistry;
		return m_cronAdapter.IsChatSessionBusy(sessionKey, context);
	}
