...
host.m_dispatcher.Register("gateway.agents.update", [&host](const protocol::RequestFrame& request) {
		...
		host.m_dispatcher.Register("gateway.channels.accounts", [&host](const protocol::RequestFrame& request) {
			const std::string channelFilter = RequestParamsView(request.paramsJson).GetString("channel");
			const auto accounts = host.m_channelRegistry.ListAccounts();
		...
		host.m_dispatcher.Register("gateway.channels.status.get", [&host](const protocol::RequestFrame& request) {
			const std::string channel = RequestParamsView(request.paramsJson).GetString("channel");
			const auto statuses = host.m_channelRegistry.ListStatus();
		...
		host.m_dispatcher.Register("gateway.channels.status.exists", [&host](const protocol::RequestFrame& request) {
			const std::string channel = RequestParamsView(request.paramsJson).GetString("channel");
			const auto statuses = host.m_channelRegistry.ListStatus();
			...
			return protocol::OkResponse(request, "{\"channel\":\"" + EscapeJsonString(channel.empty() ? "*" : channel) +
					"\",\"exists\":" + std::string(exists ? "true" : "false") + "}");
			});

		host.m_dispatcher.Register("gateway.channels.status.count", [&host](const protocol::RequestFrame& request) {
			const std::string channel = RequestParamsView(request.paramsJson).GetString("channel");
			const auto statuses = host.m_channelRegistry.ListStatus();
		...
		host.m_dispatcher.Register("gateway.channels.route.resolve", [&host](const protocol::RequestFrame& request) {
			const std::string channel = RequestParamsView(request.paramsJson).GetString("channel");
			const std::string account = RequestParamsView(request.paramsJson).GetString("accountId");
			const std::string sessionId = RequestParamsView(request.paramsJson).GetString("sessionId");
			const std::string agentId = RequestParamsView(request.paramsJson).GetString("agentId");
			const ChannelRouteEntry route = host.m_channelRegistry.ResolveRoute(channel, account);
			const SessionEntry session = host.m_sessionRegistry.Resolve(sessionId.empty() ? route.sessionId : sessionId);
			const AgentEntry agent = host.m_agentRegistry.Get(agentId.empty() ? route.agentId : agentId);
