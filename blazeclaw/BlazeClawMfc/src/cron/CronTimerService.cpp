...
namespace blazeclaw::cron {
	...
	namespace {
		...
		// Phase DI Step 6 target: cron/transport-dispatch reproducible integration tests.
		WebhookDispatchResult DispatchWebhookPostWinHttp(
			const std::string& url,
			const std::int64_t timeoutMs = 5000) {
			WebhookDispatchResult result;

			if (test_hooks::HasWebhookTransportDispatchStub()) {
				const auto stubResult = test_hooks::InvokeWebhookTransportDispatchStub(url, timeoutMs);
				result.attempted = stubResult.attempted;
				result.httpStatus = stubResult.httpStatus;
				result.error = stubResult.error;
				return result;
			}
		...
		using RunOutcome = CronTimerRunOutcome;
