...
const MAX_PERSONAL_REMINDER_DELAY_MS = 1000 * 60 * 60 * 24 * 30; // 30天上限

// 关键词检测：消息中是否包含提醒意图
const READABLE_REMINDER_KEYWORD_RE = /(提醒|叫我|通知|告诉|闹钟|定时|remind me|timer)/iu;

// 群提醒/群任务排除（这些走群任务路径，不创建个人提醒）
const READABLE_GROUP_REMINDER_RE = /(大家|全群|所有人|同学们|家长们|全体|每个人|所有家长|所有同学)/u;
const READABLE_GROUP_TASK_RE = /(通知全群|群公告|全群(作业|活动|公告)|所有人.*(提交|上传|回执)|大家.*(提交|上传|回执))/u;

// 相对时间：1分钟后、半小时后、2小时后、3天后
const READABLE_RELATIVE_REMINDER_RE = /(\d+|半|[零一二两三四五六七八九十]{1,3})\s*(秒钟|秒|分钟|分|小时|时|天)\s*(?:之后|以后|后)/u;

// 绝对日期：今天、明天、后天、大后天
const READABLE_DAY_RE = /(今天|明天|后天|大后天)/u;

// 星期：本周三、下周五、周日
const READABLE_WEEK_RE = /(本周|这周|下周)?\s*(?:周|星期|礼拜)([一二三四五六日天1-7])/u;

// 时间：下午3点、晚上8点半、凌晨2点
const READABLE_CLOCK_RE = /(凌晨|早上|上午|中午|下午|晚上|夜里)?\s*(\d{1,2})(?:[：:点时](\d{1,2})?\s*分?)?/u;

const DEFAULT_ABSOLUTE_REMINDER_HOUR = 9;
const DEFAULT_ABSOLUTE_REMINDER_MINUTE = 0;
...
function parseChineseInteger(value) {
  ...
  if (tenIdx >= 0) {
    const left = raw.slice(0, tenIdx);
    const right = raw.slice(tenIdx + 1);
...
function tryParseRelativeReminder(text, nowMs) {
  const match = READABLE_RELATIVE_REMINDER_RE.exec(text);
  if (!match) return null;
  const amount = parseChineseInteger(match[1]);
  if (!amount || amount <= 0) return null;
  const delayMs = delayMsForUnit(amount, match[2]);
  if (!Number.isFinite(delayMs) || delayMs <= 0 || delayMs > MAX_PERSONAL_REMINDER_DELAY_MS) return null;
  const title = cleanReminderTitle(text, match.index, match[0]);
  return {
    title: title || '这件事',
    dueAt: nowMs + delayMs,
    delayMs,
...
function parseClockText(text) {
  const match = READABLE_CLOCK_RE.exec(text);
  if (!match) return null;
  ...
  if (hour === 24) hour = 0;
  return { hour, minute, matchText: match[0] };
}
...
function tryParseAbsoluteReminder(text, nowMs) {
  const dayMatch = READABLE_DAY_RE.exec(text);
  const weekMatch = READABLE_WEEK_RE.exec(text);
  ...
  if (dayMatch) {
    ...
    const weekdayMap = { '日': 0, '天': 0, '7': 0, '一': 1, '1': 1, '二': 2, '2': 2, '三': 3, '3': 3, '四': 4, '4': 4, '五': 5, '5': 5, '六': 6, '6': 6 };
    const targetWeekday = weekdayMap[weekMatch[2]];
  ...
  const clock = parseClockText(text.replace(dateText, ' '));
  const due = new Date(targetDay);
  ...
  const title = cleanAbsoluteReminderTitle(text, dateText, clock ? clock.matchText : null);
  const delayLabel = due.toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' });
  return {
    title: title || '这件事',
    dueAt: due.getTime(),
...
export function tryParseReminderDraft(rawText, nowMs) {
  ...
  const text = normalizeMentionText(rawText);
  ...
  // 优先匹配相对时间
  const relative = tryParseRelativeReminder(text, nowMs);
  if (relative) return relative;

  // 绝对时间
  const absolute = tryParseAbsoluteReminder(text, nowMs);
