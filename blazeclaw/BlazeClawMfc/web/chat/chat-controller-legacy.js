(function () {
    const LEGACY_CONTROLLER_API_METHOD_SURFACE = [
        "nextId",
        "post",
        "flushQueue",
        "request",
    ...
    const LEGACY_REGRESSION_CHECK_BASELINE = [
        "config coercion parity fixtures",
        "config form-utils parity fixtures",
        "assistant guard disconnected",
        "exec approval token parser",
        "exec approval approve action",
        "exec approval malformed-token guard",
        "exec approval deny + expired actions",
        "assistant session-key propagation",
        "assistant normalization defaults",
        "assistant error retention",
        "control-ui bootstrap snapshot defaults",
        "control-ui bootstrap refresh retention",
        "assistant stale-response suppression",
        "config submit path smoke",
        "speech transcript CJK repeat quality gate",
        "structured transcript smoke",
        "structured content degraded-text rendering",
        "detached send semantics",
        "session subscribe + compaction controls",
        "stream final transcript commit",
        "history reconcile repair-only",
        "scope-aware rpc error formatting",
        "abort fallback reconcile recovery",
        "abort fallback unresolved diagnostics",
        "run-id extraction tolerant envelope compatibility",
        "queue waiting-status guardrail",
        "cron slash execution bridge callback",
        "cron slash hint hidden when disabled",
        "cron slash hint visible when enabled",
        "watchdog poll event forwarding",
        "speech lifecycle stale interim guard",
        "speech final transcript authority",
        "speech final stale preview response guard",
        "speech final nested transcript authority",
        "speech final missing transcript does not reuse preview text",
        "speech final lifecycle preview invalidation",
        "speech final dispatch blocks late preview lifecycle",
        "speech preview terminal remains active",
        "speech preview empty response remains non-terminal",
        "speech transcribe no-speech classification and status guidance parity",
    ];

    function getBaselineInventorySnapshot() {
        return {
            baselineVersion: "phase0.0",
            controllerApiMethodSurface: LEGACY_CONTROLLER_API_METHOD_SURFACE.slice(),
            regressionChecks: LEGACY_REGRESSION_CHECK_BASELINE.slice(),
        };
    }
    ...
    function createControllerCompatibilityFacade(implementation) {
        ...
        const legacyApiMethods = LEGACY_CONTROLLER_API_METHOD_SURFACE;
