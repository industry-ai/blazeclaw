...
6. **Control-plane breadth/depth** — BlazeClaw now has WebView tabs for **agents/channels/cron/dreaming/nodes/instances/usage/files/skills/observability**, but these are intentionally lightweight and still behind OpenClaw’s Lit control-plane depth (especially deeper **debug/logs** UX, **devices**, richer **exec approvals/config** governance, and richer settings/config UX).
...
| **Still missing controller surfaces** | N/A | `devices.ts`, full `exec-approvals.ts` config surface, and deeper `debug.ts` / `logs.ts` / `sessions.ts` / `exec-approval.ts` UX parity |
