...
| **`agent-files.ts`** | Workspace file list/get/set | `agents.files.list`, `agents.files.get`, `agents.files.set` | `web/chat/agents-controller.js` + `web/chat/index.html` (**partial parity implemented**: list/get are wired via `gateway.agents.files.list` and `gateway.agents.files.get`; `set` flow remains pending) |
...
| **`debug.ts`** | Operator debug panel | `status`, `health`, `models.list`, `last-heartbeat`, arbitrary method call | **None** |
| **`devices.ts`** | Device pairing | `device.pair.list`, `device.pair.approve`, … | **None** |
...
| **`health.ts`** | Health summary | `health` | Indirectly via lifecycle line in chat; no dedicated health tab |
| **`logs.ts`** | Log tail viewer | `logs.tail` | **None** |
...
| **`skills.ts`** | Skills report, edit, ClawHub search/install | `skills.*`, hub methods | `web/chat/agents-controller.js` + `web/chat/index.html` (**partial parity implemented**: skills status/report and commands surfaced via `skills.status`/`gateway.skills.status` + canonical **`skills.commands`** (alias → `gateway.skills.commands`); edit/install/ClawHub flows remain pending) |
...
6. **Control-plane breadth/depth** — BlazeClaw now has WebView tabs for **agents/channels/cron/dreaming/nodes/instances/usage/files/skills**, but these are intentionally lightweight and still behind OpenClaw’s Lit control-plane depth (especially **sessions**, **debug**, **devices**, **logs**, **exec approvals**, and richer settings/config UX).
...
| **Agents control-plane shell** | Multi-panel shell in `web/chat` (`overview/tools/files/skills/channels/cron/dreaming/nodes/instances/usage`) with panel persistence and regressions | Reach full OpenClaw Lit parity depth and UX completeness |
| **Agent identity + skills** | `agent.identity.get` lifecycle wiring; `skills.status`/`gateway.skills.status`; commands surfaced | Skills edit/install/ClawHub workflows |
| **Agent files** | `gateway.agents.files.list` + `gateway.agents.files.get` wired in WebView | `agents.files.set` equivalent and richer editing/file ops UX |
| **Channels** | Canonical `channels.status`, `web.login.start`, `web.login.wait`, `channels.logout`; snapshot state + action lifecycle | Optional UX depth and broader channel-specific workflows |
| **Cron** | `cron.status/list/add/update/remove/run/runs`, form validation, read/mutation loops | Additional hardening and backend behavior depth as needed |
| **Dreaming** | `doctor.memory.status/dreamDiary/backfill/reset*`, `config.patch`, `config.schema.lookup`, dreaming panel flows | Iterative UX and payload-depth polish |
| **Nodes + Presence + Usage** | `node.list`, `system-presence`, `sessions.usage*` tabs, loaders, stale guards, regressions | Additional operations/depth beyond current read-focused baseline |
...
| **Still missing controller surfaces** | N/A | `debug.ts`, `devices.ts`, `logs.ts`, full `exec-approvals.ts` config surface, and deeper `sessions.ts` / `exec-approval.ts` UX parity |
