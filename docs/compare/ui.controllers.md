...
1. **Message transcript model (intentional delta; tracked as polish)** — OpenClaw keeps **`chatMessages`** as structured assistant/user objects (content blocks, timestamps). BlazeClaw now stores a structured transcript and has a transcript-driven render path behind feature flag, but default UX still emphasizes flattened bubbles; richer tool/rich-content rendering remains optional polish.
2. **Gateway client parity (intentional embedded-vs-shim split)** — OpenClaw uses **device auth**, **scopes**, **reconnect** semantics, and **`GatewayBrowserClient`** feature flags. BlazeClaw WebView intentionally keeps a narrow embedded RPC bridge for desktop mode; shim-mode expectations and scope-failure messaging are documented and aligned via `scope-errors.js`.
...
5. **Exec approvals / operator tools (baseline complete; governance depth pending)** — OpenClaw **`app-gateway.ts`** integrates **`exec-approval.ts`** with the host; BlazeClaw provides a WebView baseline approval queue (token detect + approve/deny action) for `email.schedule`, while richer policy/config surfaces remain product follow-up.
6. **Control-plane breadth/depth (baseline complete; polish backlog)** — BlazeClaw ships WebView tabs for **agents/channels/cron/dreaming/nodes/instances/usage/files/skills/observability/devices**; remaining deltas are explicit polish/backlog items rather than Tier-1 parity blockers.
...
2. **Chat event semantics** — **Done for current scope:** cross-run finals respect **`NO_REPLY`**; terminal final/aborted paths commit structured transcript entries directly; parity matrix in [`CHAT_EVENTS_SEMANTICS_MATRIX.md`](./CHAT_EVENTS_SEMANTICS_MATRIX.md). Optional additional golden fixture replay remains backlog, not a parity gate.
...
4. **Incremental controller ports** — **Done for baseline scope:** high-value controller baselines are landed through Phases E-K; remaining items in [`UI_CONTROLLERS_PHASE0_DECISION_TRACKER.md`](./UI_CONTROLLERS_PHASE0_DECISION_TRACKER.md) are explicit product-depth follow-ons.
...
Last updated: 2026-04-22 (gap suggestions 1–5 are now reconciled for current scope: canonical chat bootstrap RPCs + gateway aliases, `NO_REPLY` cross-run/final semantics + matrix alignment, structured transcript schema/writer hardening with transcript-render flag path + degraded rich-content text markers, detached `/btw` side-channel flow end-to-end, Phase D embedded-vs-shim docs + scope-aware RPC messaging, Phase E sessions baseline completed, **Phase F exec-approval baseline implemented**, **Phase G observability baseline implemented**, **Phase H devices baseline implemented**, **Phase I skills-depth baseline implemented**, **Phase J agent-files write baseline implemented**, **Phase K control-plane depth/polish triage completed**, **Phase L harness decision completed** (retain `runRegressionChecks` + gateway/Catch2), and **Phase M doc reconciliation completed through M.4**; named reviewer smoke attestation (M.5) remains pending)
