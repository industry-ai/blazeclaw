...
| 2026-04-22 | N.P3 (planned) | Reopened deep semantic closure for remaining `partial` families despite route-level parity completion. Added detailed step-by-step closure backlog for `device.pair.*`, `config.apply/patch` depth, `tools.effective`, `skills/update` depth, `tts.*`, `secrets.*`, and system/event envelope alignment. | `docs/compare/OPENCLAW_SERVER_METHODS_DEEP_MECHANICAL_AUDIT_AND_OPTIMIZATION.md` |
