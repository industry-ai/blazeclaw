...
Generated: 2026-05-18 (reconciled against current BlazeClaw cron sources with Phase U Step 8/10 wake-mode alias canonicalization and prior Phase T Step 4/10 cross-layer alignment)
...
4. **Gateway normalization flow** — `cron.add`/`cron.update` normalize inside `CronOpsService` after gateway request validation, and `GatewayHost::HandleInboundText` now applies pre-validator canonicalization for legacy/flat payloads across `cron.add`/`cron.update`/`cron.run`/`cron.runs`/`wake`; latest increments deepen `cron.update` flat schedule/payload/delivery alias lifting into nested patch structure, canonicalize flat `failureAlert*` aliases into nested `failureAlert` objects for add/update patch paths, and canonicalize legacy wake-mode alias values to strict `next-heartbeat` before validation. Remaining gap is broader tool-surface action depth, not absence of gateway normalization bridge.
